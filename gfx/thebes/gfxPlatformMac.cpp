...
void gfxPlatformMac::RegisterSupplementalFonts() {
  ...
  void gfxPlatformMac::GetCommonFallbackFonts(
  ...
  static CVReturn VsyncCallback(
      CVDisplayLinkRef aDisplayLink, const CVTimeStamp* aNow,
      const CVTimeStamp* aOutputTime, CVOptionFlags aFlagsIn,
      CVOptionFlags* aFlagsOut, void* aDisplayLinkContext);
  ...
  class OSXVsyncSource final : public VsyncSource {
   ...
   private:
    static void DisplayReconfigurationCallback(
        ...
        void* aUserInfo) {
      static_cast<OSXVsyncSource*>(aUserInfo)->OnDisplayReconfiguration(
          aDisplay, aFlags);
    }
