 ...
 */
class EventDispatcher final : public nsIAndroidEventDispatcher {
 public:
  NS_DECL_ISUPPORTS
  ...
  void Attach(id aDispatcher, nsPIDOMWindowOuter* aDOMWindow);
  void Detach();

  nsresult Dispatch(const char16_t* aEvent, id aData = nullptr,
                    nsIAndroidEventCallback* aCallback = nullptr);

  bool HasListener(const char16_t* aEvent);
  bool HasGeckoListener(CFStringRef aEvent);
  void DispatchToGecko(CFStringRef aEvent, id aData, id aCallback);

  static nsresult UnboxBundle(JSContext* aCx, id aData,
 ...
 private:
  ...
  id WrapCallback(nsIAndroidEventCallback* aCallback,
